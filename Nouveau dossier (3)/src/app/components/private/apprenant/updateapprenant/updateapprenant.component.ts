...
@Component({
  selector: 'app-updateapprenant',
  templateUrl: './updateapprenant.component.html',
  styleUrls: ['./updateapprenant.component.css']
})
export class UpdateapprenantComponent implements OnInit {
  ...
  ngOnInit(): void {  let iduser = this.router.snapshot.params.id;
  ...
  get cnom(){ 
   ...
   saveupdate() {
    ...
    let apprenant = new User( iduser ,data.cnom, data.cprenom, data.ccin, data.cemail, data.ctel, data.cdatanaissance, data.cmotdepasse,"apprenant");
    this.serviceuser.updateuser(apprenant).subscribe(
      result => { this.route.navigate(['/listeapprenant']) },
      errors => { console.log(errors) }
